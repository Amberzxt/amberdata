function Isosurfaces( volume, isovalue, vert_name, frag_name, color_name, rainbow_color)
{
        ...
        for ( var y = 0; y < volume.resolution.y - 1; y++ )
        {
            for ( var x = 0; x < volume.resolution.x - 1; x++ )
            {
                ...
                for ( var j = 0; lut.edgeID[index][j] != -1; j += 3 )
                {
                    ...
                    /*var S0 = parseInt((scalars[0]-0.1)/0.7*255);
                    var S1 = parseInt((scalars[1]-0.1)/0.7*255);
                    var S2 = parseInt((scalars[2]-0.1)/0.7*255);
                    
                    var C0 = new THREE.Color().setHex( cmap[ S0 ][1] );
                    var C1 = new THREE.Color().setHex( cmap[ S1 ][1] );
                    var C2 = new THREE.Color().setHex( cmap[ S2 ][1] );
                    geometry.faces[face_count].vertexColors.push( C0 );
                    geometry.faces[face_count].vertexColors.push( C1 );
                    geometry.faces[face_count].vertexColors.push( C2 );
                    face_count++;*/
                }
            }
            cell_index++;
